...
namespace AIHelper.Manage.Rules.ModList
{
    internal class RuleModForIpa : ModListRulesBase
    {
        ...
        internal override bool Condition()
        {
            ...
            return (!Path.Combine(ManageSettings.CurrentGameModsDirPath, ModlistData.ModName, "IPA").IsNullOrEmptyDirectory("*.dll", null, true)
                ||
                !Path.Combine(ManageSettings.CurrentGameModsDirPath, ModlistData.ModName, "Plugins").IsNullOrEmptyDirectory("*.dll", null, true)
                )
                && !File.Exists(Path.Combine(ManageSettings.CurrentGameModsDirPath, ModlistData.ModName, "IPA", "Data", "Managed", "IllusionInjector.dll"))
                && !File.Exists(Path.Combine(ManageSettings.CurrentGameModsDirPath, ModlistData.ModName, "BepInEx", "patchers", "BepInEx.IPAVirtualizer.dll"));
        }
