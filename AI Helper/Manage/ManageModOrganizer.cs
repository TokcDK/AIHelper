...
namespace AIHelper.Manage
{
    static partial class ManageModOrganizer
    {
        ...
        public static void PreloadingSetup()
        {
            ...
            int index = 1;
            var forcedLibrariesSectionName = ManageSettings.MOProfileSettingsIniForcedLibrariesSectionName;
        ...
        /// <param name="modname"></param>
        /// <param name="activate">If need the <paramref name="modname"/> was activated in modlist</param>
        /// <param name="modNameToInsertWith">Name of mod after which must be inserted else will me inserted in the end od list.</param>
        /// <param name="placeAfter"> Will determine before or after of <paramref name="modNameToInsertWith"/> new <paramref name="modname"/> must be inserted</param>
        public static void ActivateDeactivateInsertMod(string modname, bool activate = true, string modNameToInsertWith = "", bool placeAfter = true)
        {
            // insert new
            var modList = new ModlistData();
        ...
        {
            var moFilesForClean = ManageSettings.MoFilesForCleanList;
            ...
            foreach (var file in moFilesForClean)
            {
                ...
                TryRemoveMOFileOrDirectory(path);
            }
        }

        private static void TryRemoveMOFileOrDirectory(string path)
        {
            bool isDir = path.EndsWith("\\", StringComparison.InvariantCulture) && Directory.Exists(path);
            path = isDir
                ? path.TrimEnd('\\')
                : path;
            ...
            foreach (FileSystemInfo foundFile in searchDir.EnumerateFiles(searchPattern))
            {
                try
                {
