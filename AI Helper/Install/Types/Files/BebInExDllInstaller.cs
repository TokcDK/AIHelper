...
namespace AIHelper.Install.Types.Files
{
    class BebInExDllInstaller : FilesInstallerBase
    {
        ...
        protected override bool Get(FileInfo dllfile)
        {
            ...
            if ((!string.IsNullOrEmpty(name) && name.Substring(0, 1) == "[" && !name.StartsWith("["+ManageSettings.Games.Game.GameAbbreviation+"]", StringComparison.InvariantCultureIgnoreCase)) || (name.Length >= 5 && name.Substring(0, 5) == "["+ ManageSettings.Games.Game.GameAbbreviation + "][") || ManageStrings.IsStringAContainsStringB(name, author))
            {
            }
